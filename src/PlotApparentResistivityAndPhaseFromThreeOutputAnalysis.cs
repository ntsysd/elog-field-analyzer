...
{
    class PlotApparentResistivityAndPhaseFromThreeOutputAnalysis : PlotApparentResistivityAndPhase
    {
        public PlotApparentResistivityAndPhaseFromThreeOutputAnalysis()
        {
        }
            ...
            try
            {
                ...
                using (StreamReader reader = new StreamReader(@csvFileName))
                {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        var dZxx = errorFactor * Convert.ToDouble(values[17]);
                        var dZxy = errorFactor * Convert.ToDouble(values[18]);
                        var dZyx = errorFactor * Convert.ToDouble(values[19]);
                        var dZyy = errorFactor * Convert.ToDouble(values[20]);
